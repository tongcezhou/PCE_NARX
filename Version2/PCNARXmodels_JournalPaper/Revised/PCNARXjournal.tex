...
\title{Metamodeling of Dynamic Nonlinear Structural Systems through Polynomial Chaos NARX Models}
 ...
 \cortext[cor2]{Corresponding author, Tel/Fax: (+ 41) 44 633 6755}
...
\begin{keyword}
Metamodels \sep Polynomial chaos expansion\sep NARX modeling \sep Finite element models\sep Dynamic response\sep Nonlinear hysteretic behavior \sep Structural systems.
\end{keyword}
...
\eeq
%%
with $t = 1,2,\ldots, N $ designating normalized by the sampling period discrete time, $x[t,\bxi_X]$ the excitation input signal, $y[t,\bxi]$ the corresponding numerical model response signal, and $\bxi = [ \bxi_S^{T} \ ,\ \bxi_X^{T} ]^{T}$ the $M$-dimensional ($M = M_1 + M_2$) complete vector of input random variables with known joint probability density function (pdf) $f(\bxi)$. \textcolor{red}{Such a pdf may be either prespecified, e.g. for the case that a structure under design has to be simulated for a selected range of values of its geometric properties, or empirically estimated from statistical data of a manufacturing process when it concerns material properties. For the case of excitation input variables the underlying pdfs may be estimated by fitting parametric models onto historical data. For example, statistical data extracted from the PEER earthquake database have been fitted to an appropriate parametric model in order to produce synthesized earthquakes for the purposes of simulation in \cite{Spiridonakos-Chatzi2013a}. Finally, in the case of dependency among the input variables, the copula function may be used for the description of this dependency and the construction of the proper PC basis \cite{Blatman-Sudret2010}.}
...
%%
\beq \tilde{y}[t,\bxi] =  \sum_{i=1}^{n_\vartheta} \vartheta_i(\bxi) \cdot g_i({\bld z}[t,\bxi]) + e[t,\bxi] \label{eq:PCNARX}\eeq
%%
where $n_\vartheta$ is the number of nonlinear model terms $g_i({\bld z}[t,\bxi])$ that are generated from the regression vector ${\bld z}[t,\bxi] = \left\{ \tilde{y}[t-1,\bxi], \ldots, \tilde{y}[t-n_a,\bxi],  x[t,\bxi_X],\ldots ,x[t-n_b,\bxi_X] \right\}^T$ with $n_a, n_b$ designating the maximum output and input time lags, respectively, and $e[t,\bxi]\! \sim\! \text{NID} (0,\sigma^2_e,(\bxi))$ the model's residual sequence with NID$(\cdot,\cdot)$ denoting normally independently distributed process with the indicated mean and variance. It should be mentioned that the model terms $g_i({\bld z}[t,\bxi])$ may be constructed from a variety of local or global basis functions including polynomials, splines, neural networks, wavelets and others \cite{Wei-Billings2009}. \textcolor{red}{Furthermore, even if the case of univariate responses is presently considered, the more general multivariate case, that is the metamodelling for a number of outputs could readily be obtained by utilizing nonlinear vector ARX models, at the expense of increased computational cost.}

The important feature of PC-NARX models, in comparison with the conventional NARX models \cite{Chen-Billings1989}, is that they are characterized by parameters $\vartheta_i(\bxi)$ which are random variables themselves. The latter are actually represented by a deterministic mapping which describes their relationship to the input random variables. More specifically, assuming that the PC-NARX model parameters $\theta_i(\bxi)$ have finite variance, they admit the following polynomial chaos representation \cite{Soize-Ghanem2004}:
%%
\beq 
...
More specifically, it is considered that a series of $K$ simulations conducted for a corresponding number of input random vector realizations $\bxi_k = [\xi_{k,1}, \xi_{k,2}, \ldots ,\xi_{k,M}]^T$ (with $k=1,2,\ldots,K$), and the resulting set of excitation signals ${\bld x}_k^N = \{ x_k[1], x_k[2], \ldots, x_k[N] \}$ are available. It is noted that for the purposes of notational simplicity the dependency of the input and output signals on $\bxi_k$ is not indicated explicitly in the following relationships.

The corresponding dynamic response of the full scale numerical model is indicated as ${\bld y}_k^N = \{ y_k[1], y_k[2],$ $\ldots, y_k[N] \}$ and as already mentioned it is assumed to follow the general PC-NARX model of Eq. (\ref{eq:PCNARX}):
%%
...
As already mentioned, a metamodel that may replace the numerical model for additional simulations is sought. \textcolor{red}{That is the metamodel should be able to provide accurate description of the structural dynamics and the requested responses by using only the time-series of the excitation. Toward this end, the estimation of the parameter vector $\bth$ may be based upon the minimization the Simulation Error (SE) criterion.} 

\textcolor{red}{On the other hand, for problems where direct feedback of response is required in real-time, such as vibration control, structural health monitoring, model updating and others, a metamodel that could provide short-term predictions, usually one-step-ahead, may be of interest. In such a case, both the input excitation and past response measurements are provided in the metamodel representation. Such a metamodel is thus estimated based on the minimization of the prediction error.}
...
\eeq
%%
with given initial conditions $\{ y_k[1 - n_a], \ldots, y_k[0] \}$ and $\{ x_k[1-n_b], \ldots, x_k[0]\}$. The responses $\tilde{y}_k[t]$ of the metamodel should be as close as possible to that of the numerical model $y_k[t]$, thus the estimation of a PC-NARX metamodel may be based on the minimization of the following SE criterion: 
%%%
...
\end{align}
%%%
This optimization problem may be solved by iterative nonlinear optimization methods. However, these methods are typically amenable to wrong convergence problems when initialized by arbitrary or inaccurate estimates. For this reason, the SE method may be initialized by the parameter vector $\bth$ obtained through the Prediction Error (PE) method described in the following section.
...
\begin{figure}[t!]
\begin{center}
{\begin{picture}(420,340)
\put(0,0){\includegraphics[width = 0.9\textwidth]{figs/cubic_randexci_val_error.pdf}}
\put(0,330){(a)}
\put(0,160){(b)}
\end{picture}}
\caption{Cubic stiffness system PC-NARX metamodel validation: PC-NARX metamodel simulation contrasted to the simulations of the numerical model and the respective simulation error for (a) $k=3750,\sigma_x = 3750$, and (b) $k = 5000,\sigma_x = 5000$. 
...
The displacement-restoring force diagrams for two of the simulation experiments ($\#2$ and $\#15$) are depicted in Fig. \ref{fig:boucwen_randexc}. For these simulation experiments the sets of input variables are $\bxi_2 = [\beta , \sigma_x]^T \simeq [ 0.08, 20]^T $ and $\bxi_{15} \simeq [0.86 , 91]^T $, and obviously a range of behaviors from almost linear to highly nonlinear may be observed depending on the sampled values of the random input variables.
...
\begin{center}
{\begin{picture}(420,340)
\put(0,0){\includegraphics[width = 0.9\textwidth]{figs/boucwen_randexci_val_error.pdf}}
\put(0,330){(a)}
\put(0,160){(b)}
\end{picture}}
\caption{Hysteretic system PC-NARX metamodel validation: PC-NARX metamodel simulation contrasted to the simulations of the numerical model and the respective simulation error for (a) $ \beta = 0.25,\sigma_x = 50$, and (b) $ \beta = 1,\sigma_x = 100$. 
...
The time-histories of the model's displacement response as calculated by the FE model for node 7 versus the total reaction base force for the simulations with the input excitation lowest standard deviation value (simulation experiment $\#8$) and the highest standard deviation value (simulation experiment $\#15$) are shown in Figure \ref{fig:SF_randexc}. As expected, significant differences between the two datasets may be readily observed, fact that may attributed to the amplitude of excitation which as becomes higher excites the nonlinear dynamics of the modeled structure.
...
%  
with ${\bld z}[t] = \left\{ \ y[t-1], y[t-2],\ldots,y[t-14], x[t], x[t-1],x[t-2] ,\ldots, x[t-14]\ \right\}^{T}$, $\ell_1 \in \{1,2,3\}$, $\ell_2 \in\{ 1,\ldots,7 \}$, and $\ell_3 \in\{ 0,1 \}$, leading to 697 candidate terms including the constant function. The absolute function is incorporated in the above set of functions for the modeling of the physical problem at hand, since the bilinear material behavior is expected to add hysteretic phenomena into the dynamics of the modeled structure.  
...
\caption{Shear frame FE model metamodeling: Backward selection of the polynomial chaos functional subspace based on the mean adjusted $R^2$ criterion (maximum total polynomial degree $P=4$; initial subspace selected through GA with the corresponding $\overline{R^2_\text{adj}}$ value obtained being indicated by the red dashed-line).} \label{fig:SF_MSS_B}
\end{center}
\end{figure}
% ================================================
...
\begin{center}
{\begin{picture}(420,340)
\put(0,0){\includegraphics[width = 0.9\textwidth]{figs/SF_randexci_val_error.pdf}}
\put(0,330){(a)}
\put(0,160){(b)}
\end{picture}}
\caption{Shear frame FE model PC-NARX metamodel validation: PC-NARX metamodel simulation contrasted to the simulations of the FE model and the respective simulation error for (a) $ E = 188$ GPa, $\sigma_a = 12$ (m/s$^2$), and (b) $ E = 212$ GPa ,$\sigma_a = 4$ (m/s$^2$). 
...
% ================================================
The research described in this work was financially supported by the Swiss Federal Roads Office � FEDRO (Bundesamt f�r Strassen - ASTRA ), Project \# AGB 2012/015.
%--------------------------------------------------------------------------------------------------
%                                   Bibliography
%--------------------------------------------------------------------------------------------------
