...
! ================================
*DIM,EXCIT,ARRAY,NoExp*NoS,2
*CREATE,ansuitmp                                                                                                                    
*VREAD,EXCIT(1),'TestInput','txt',,IJK,NoExp*NoS,2
(E16.7)                                                                                                                                                                
*END                                                                                                                                                                   
/INPUT,ansuitmp
...
*DO,J,1,NoExp			! Experiments DO LOOP
	...
	STRin = STRCAT('MultiII_Test_',CHRVAL(J))
	STRd = STRCAT('MultiII_Test_dspl_',CHRVAL(J))
	STRv = STRCAT('MultiII_Test_vel_',CHRVAL(J))
	STRa = STRCAT('MultiII_Test_acc_',CHRVAL(J))
	...
	! Input Force
	Finput = EXCIT(1,2)
	F,2,FX,Finput,,8,2		! 1st floor
	F,9,FX,Finput*2,,12,1	! 2nd floor
	F,13,FX,Finput*3,,16,1	! 3rd floor
	F,17,FX,Finput*4,,18,1	! 4th floor
	F,19,FX,Finput*5,,20,1	! 5th floor
	...
	*DO,I,1,NoS				! Time index DO LOOP
		...
		Gacc = EXCIT((J-1)*NoS+I,1)
		! X-axis input force
		Finput = EXCIT((J-1)*NoS+I,2)
		...
		*DO,ElemI,1,18				! Time index DO LOOP
			STRx = STRCAT('MultiII_Test_Sx_',CHRVAL(ElemI))
			STRx = STRCAT(STRx,'_')
			STRx = STRCAT(STRx,CHRVAL(J))
			! X stresses at element 1, node I at 8 integration points  
			*GET,SIGMAX(1),ELEM,ElemI,NMISC,1
			*GET,SIGMAX(2),ELEM,ElemI,NMISC,2
			*GET,SIGMAX(3),ELEM,ElemI,NMISC,3
			*GET,SIGMAX(4),ELEM,ElemI,NMISC,4
			*GET,SIGMAX(5),ELEM,ElemI,NMISC,5
			*GET,SIGMAX(6),ELEM,ElemI,NMISC,6
			*CFOPEN,STRx,txt,,APPEND		! Open txt file
			*VWRITE,Tindx,Gacc,SIGMAX(1),SIGMAX(2),SIGMAX(3),SIGMAX(4),SIGMAX(5),SIGMAX(6)
(8(E16.7,2X))
			*CFCLOS	
		*ENDDO
